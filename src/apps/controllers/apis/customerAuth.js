...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    // Mặc định: gửi mật khẩu tạm thời trực tiếp qua email
    // Nếu client muốn link thay vì mật khẩu trực tiếp, gửi { mode: 'link' }
    if (!mode || mode !== 'link') {
      // Mặc định: gửi mật khẩu tạm thời trực tiếp qua email (không gửi link)
      const tempPassword = crypto.randomBytes(4).toString('hex'); // 8 ký tự hex
      const hashedPassword = await bcrypt.hash(tempPassword, 10);

      // Cập nhật mật khẩu mới ngay lập tức
      customer.password = hashedPassword;
      await customer.save();

      // Vô hiệu hoá token cũ và xóa token lưu trong DB (nếu có)
      await addTokenBlackList(customer._id);
      await deleteCustomerToken(customer._id);

      // Gửi email chứa mật khẩu tạm thời
      const mailTemplatePathDirect = `${config.get("mail.mailTemplate")}/mail-resetPassDirect.ejs`;
      await sendMail(mailTemplatePathDirect, {
    ...
    // Nếu client yêu cầu link thay vì mật khẩu trực tiếp
...
exports.changePassword = async (req, res) => {
  try {
    ...
    // Lấy lại customer với password từ DB
