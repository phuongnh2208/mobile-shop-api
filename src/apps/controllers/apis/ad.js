...
const updateAd = async (Model, req, res, adType) => {
    ...
    try {
        // Lấy document hiện tại để biết image cũ và vị trí
        const adDoc = await Model.findById(id);
        if (!adDoc) {
        ...
        // Nếu có ảnh mới được upload (req.body.image), xóa ảnh cũ trên đĩa nếu khác
        if (image) {
            updateData.image = image;
            try {
                if (adDoc.image && typeof adDoc.image === 'string' && adDoc.image !== image) {
                    const relOld = adDoc.image.replace(/^\/+/, "");
                    const absOld = path.join(__dirname, '..', '..', '..', 'public', relOld);
                    if (fs.existsSync(absOld)) {
                        try {
                            fs.unlinkSync(absOld);
                            console.log('updateAd: removed old image', absOld);
                        } catch (e) {
                            console.warn('updateAd: failed to remove old image', absOld, e.message);
                        }
                    } else {
                        console.log('updateAd: old image not found on disk', absOld);
                    }
                }
            } catch (e) {
                console.warn('updateAd: error while removing old image', e.message);
            }
        }
...
const deleteAd = async (Model, req, res, adType) => {
    try {
        ...
        // Lấy document trước để biết image path và position
        const adDoc = await Model.findById(id);
        if (!adDoc) {
            return res.status(404).json({ message: `${adType} not found` });
        }
        ...
        // Xóa document
        const deletedAd = await Model.findByIdAndDelete(id);

        // Giảm position của tất cả banner có position > deletedAd.position
        await Model.updateMany(
            { position: { $gt: adDoc.position } },
            { $inc: { position: -1 } }
        );

        // Nếu có trường image, thử xóa file trên đĩa
        try {
            if (adDoc.image && typeof adDoc.image === 'string') {
                // adDoc.image thường dạng '/upload/banners/filename.ext'
                const rel = adDoc.image.replace(/^\/+/, ""); // 'upload/banners/filename.ext'
                const absPath = path.join(__dirname, '..', '..', '..', 'public', rel);
                if (fs.existsSync(absPath)) {
                    try {
                        fs.unlinkSync(absPath);
                        console.log('deleteAd: removed file', absPath);
                    } catch (e) {
                        console.warn('deleteAd: failed to remove file', absPath, e.message);
                    }
                } else {
                    console.log('deleteAd: file not found on disk', absPath);
                }
            }
        } catch (e) {
            console.warn('deleteAd: error while removing file', e.message);
        }
