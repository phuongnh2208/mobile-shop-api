...
const handleUpload = (subfolder, options = {}) => {
  ...
  return (req, res, next) => {
    ...
    form.parse(req, (err, fields, files) => {
      ...
      // Temporary debug logs to inspect upload parsing
      try {
        console.log('upload debug: headers content-type=', req.headers['content-type']);
        console.log('upload debug: parsed fields keys=', Object.keys(fields));
        console.log('upload debug: parsed files keys=', Object.keys(files));
      } catch (logErr) {
        console.log('upload debug: logging error', logErr.message);
      }
      ...
      // Debug chosen file info
      if (file) {
        try {
          console.log('upload debug: file chosen ->', {
            originalFilename: file.originalFilename || file.name || null,
            filepath: file.filepath || file.path || file.newFilename || null,
            size: file.size || file.sizeBytes || null,
            type: file.mimetype || file.type || null,
          });
        } catch (e) {
          console.log('upload debug: file inspect error', e.message);
        }
      }
      ...
      console.log('upload debug: saved imagePath=', imagePath);
